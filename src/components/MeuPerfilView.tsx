      ...
      <div className="text-center space-y-3">
        <div className="flex justify-center">
          <div className="relative inline-block">
            <img
              src={user.avatar_url || getInitialsAvatarUrl(user.nome, 160)}
              alt={`Foto de perfil de ${user.nome}`}
              className="w-20 h-20 sm:w-24 sm:h-24 rounded-full border-4 border-card shadow-md object-cover"
            />
            <button
              onClick={onOpenAuth}
              title="Editar dados do perfil"
              className="absolute bottom-0 right-0 p-1.5 rounded-full bg-primary text-primary-foreground shadow hover:opacity-90 transition cursor-pointer"
            >
