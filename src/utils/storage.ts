...
export function getInitialsAvatarUrl(name: string, size = 128): string {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${size} ${size}" width="${size}" height="${size}">
    ...
    <rect width="${size}" height="${size}" rx="${size / 2}" fill="url(#avatarGrad)"/>
    <text x="50%" y="53%" font-family="system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif" font-size="${Math.round(size * 0.42)}" font-weight="700" fill="#ffffff" text-anchor="middle" dominant-baseline="central" letter-spacing="1">${initials}</text>
  </svg>`;
...
export function carregarUsuario(): UserProfile {
  try {
    ...
    if (raw) {
      ...
      return {
        ...parsed,
        avatar_url: getInitialsAvatarUrl(parsed.nome)
      };
    }
  } catch (e) {
    console.error(e);
  }
  const defaultNome = "Visitante Consciente";
  return {
    id: "user_default",
    nome: defaultNome,
    email: "visitante@saudeemacao.org",
    avatar_url: getInitialsAvatarUrl(defaultNome),
    logado: false
  };
}
...
export function salvarUsuario(user: UserProfile) {
  try {
    const perfilAtualizado: UserProfile = {
      ...user,
      avatar_url: getInitialsAvatarUrl(user.nome)
    };
    localStorage.setItem(STORAGE_KEY_USER, JSON.stringify(perfilAtualizado));
  } catch (e) {
    console.error(e);
  }
}
